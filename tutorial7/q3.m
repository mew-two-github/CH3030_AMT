...
l = length(time);
for i = 2:l-1
    ar_diff(i) = trapz(time(1:i),ppval(pp,time(1:i)))+trapz(time(i:l),ppval(pp,time(i:l)))-(108.6-time(i));
...
us = L/ts*60; %m/h
LUB = L*(1-t_break/ts);
MTZ = 2*LUB;%Since curve is symmetric
%% Part b
V = 8000/60/60;%m^3/s
D = sqrt(4*V/(u_superficial*pi));
t_cycle = 8;
L_used = 8*us;
L_scaleup = LUB*2 + L_used; 
sol_adsorbed = V*ci*t_cycle*3600;
avg_loading = sol_adsorbed/L_scaleup;
max_loading = sol_adsorbed/L_used;