...
e_tce = convertx(e_tce,e_a);
r_tce = convertx(r_tce,r_a);
...
tce_r = convertx(tce_r,ac_raff);
tce_e = ppval(e_p,ac_ext);
tce_e = convertx(tce_e,ac_ext);
xRN = ppval(r_p,yRN);
xRN = convertx(xRN,yRN);
%% Plotting the data
figure();
...
%% Functions to transform coords
function xnew = convertx(x,y)
    xnew = x/sin(pi/3) + y*tan(pi/6);
end